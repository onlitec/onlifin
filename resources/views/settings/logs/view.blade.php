...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
            ...
            @if($type === 'api' && !empty($entries))
                <div class="space-y-6">
                    @foreach($entries as $index => $entry)
                        <div class="border rounded-lg p-4 {{ $entry['success'] ? 'border-green-300 bg-green-50' : 'border-red-300 bg-red-50' }}">
                            <div class="flex justify-between items-start">
                                <h3 class="font-medium text-lg">
                                    Requisição #{{ $index + 1 }} - {{ $entry['provider'] }}
                            ...
                            <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <div class="bg-gray-100 p-3 rounded overflow-auto max-h-96">
                                        <pre class="text-xs">{{ json_encode($entry['request'], JSON_PRETTY_PRINT) }}</pre>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div class="bg-gray-100 p-3 rounded overflow-auto max-h-96">
                                        @if(!empty($entry['response']))
                                            <pre class="text-xs">{{ json_encode($entry['response'], JSON_PRETTY_PRINT) }}</pre>
            ...
            @elseif($type === 'laravel')
                <div class="bg-gray-100 p-3 rounded overflow-auto max-h-screen">
